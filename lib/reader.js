...
function readArt($, options){
  ...
  debug('Got %d candidates', cans.length);
  if (debug.enabled) {
    cans.forEach(function(candidate, index) {
      debug('Candidate #%d HTML: %s', index, candidate.html());
    });
  }
  ...
  if (debug.enabled) {
    debug('Top candidate content %s', topCandidate.html());
  }
  // 3nd. grab article
...
function getTopCandidate($, cans){
  ...
  cans.forEach(function(node, index){
    ...
    debug('getTopCandidate #%d score: %d', index, score);
...
function grabArticle($, topCandidate, options){
  ...
  siblings.each(function(){
    ...
    if (!append) {
      ...
      } else if (!!~allowedMedias.indexOf(tagName)) {
        append = true;
