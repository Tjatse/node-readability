 ...
 * @return {[type]}       
 */
readArt.extract = function ($, nodes, extract) {
  var retVals = [];
  nodes.each(function () {
    var item = {},
      exist = false,
    ...
    if (Array.isArray(extract)) {
      extract.forEach(function (prop) {
        if (typeof prop != 'string') {
          return;
        }
        exist = readArt.extractProp(node, prop, prop, item) || exist;
      });
    } else if (util.isObject(extract)) {
      for (var mapping in extract) {
        var prop = extract[mapping];

        if (typeof prop != 'string') {
          continue;
        }
        exist = readArt.extractProp(node, prop, mapping, item) || exist;
      }
    } else {
      exist = (item = readArt.extractProp(node, extract, extract, item)) || exist;
    }
    exist && retVals.push(item);
  });
  if (retVals.length == 1 && (typeof retVals[0] == 'string' || nodes.length == 1)) {
    return retVals[0];
  }
  return retVals;
};
 ...
 */
readArt.extractProp = function (node, prop, mapping, retVal) {
  var propVal;
  switch (prop) {
    case 'text':
      node.html(node.html().replace(/<br[^\/>]*\/?>/ig, '\\n'));
      propVal = node.text().trim().replace(/([\s\r\t\n]+)/g, ' ');
      break;
    case 'html':
      propVal = node.html();
      break;
    ...
    case 'json':
      ...
      break;
    default:
      propVal = node.attr(prop);
      break;
  }
  if (propVal) {
    retVal[mapping] = propVal;
    return true;
  }
  return false;
};
...
readArt.extractor = {
  json: function($, nodes, output){
    ...
    nodes.each(function(){
      ...
      if (children.length == 0) {
        retVal.push({
          type: 'text',
          value: readArt.extractor.text($, node, output, true)
        });
        return;
      }
      children.each(function () {
        ...
        if (child.length == 1 && child.get(0).name == 'img') {
          ...
          //find text.
          var text = readArt.extractor.text($, child, output, true)
  ...
  text: function($, nodes, output, jsonable){
    var text = '';
    nodes.each(function(){
      var node = $(this);
      if (output.break && jsonable) {
        node.html(node.html().replace(/<br[^\/>]*\/?>/ig, '\\n'));
      }
  ...
  html: function($, nodes, output, tidyAttrs){
    var html = nodes.html();
    // strip tab symbols(\r\t\n)
    if (output.stripSpaces) {
      html = html.replace(/([\r\t\n]+)/g, ' ');
