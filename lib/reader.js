...
var extRegexps = {
  positive: /article|blog|body|contenÏt|entry|main|news|pag(?:e|ination)|post|story|text/i,
...
function grabArticle ($, topCandidate, options) {
  ...
  function shouldUseParent (node) {
    return topCandidate.get(0).name.toLowerCase() !== 'article' && topCandidate.children('p').length < options.minParagraphs
  }

  function parentNotBody (node) {
    var parent = node.parent()
    return parent && parent.length > 0 && parent.get(0).name.toLowerCase() !== 'body'
  }
  if (shouldUseParent(topCandidate) && parentNotBody(topCandidate)) {
    ...
    siblings = topCandidate.parent().children()
  } else {
    // self children.
...
function getNodeWeight (node, cans, options) {
  ...
  if (textLen < options.minTextLength && (!options.scoreImg || node.find('img').length === 0)) {
    if (debugDo.enabled) {
