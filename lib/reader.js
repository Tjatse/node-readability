...
function parseImgFallback(imgFallback, node) {
  switch (typeof imgFallback) {
    case 'boolean':
      return imgFallback && node.data('src');
    case 'function':
      imgFallback = imgFallback(node);
      // fall through
    case 'string':
      if (imgFallback.indexOf('data-') === 0) {
        return node.data(imgFallback.slice(5));
      }
      return node.attr(imgFallback);
    default:
      break;
  }
}
  ...
  article.find('img,a,object,embed').each(function(){
    ...
    switch(node.get(0).name.toLowerCase()){
      case 'img':
        ...
        if (!propVal) {
          var fallbackAttr = parseImgFallback(options.imgFallback, node);
          if (fallbackAttr) {
            node.attr('src', fallbackAttr);
            propVal = fallbackAttr;
          }
        }
