...
function Article (o) {
  ...
  if (o.html) {
    ...
    var cheerioOptions = ['normalizeWhitespace', 'xmlMode', 'lowerCaseTags', 'decodeEntities']
    cheerioOptions.forEach(function (n) {
      if (typeof o.options[n] === 'boolean') {
        co[n] = o.options[n]
      }
    })
  ...
  if (typeof options.selectors === 'object') {
    ...
    Object.keys(options.selectors).forEach(function (prop) {
      ...
      Article.prototype.__defineGetter__(prop, function () {
        ...
        return (this.caches[propCacheKey] = read.extract(this.$_, this.$_(sel.selector), sel, this.options)) // eslint-disable-line no-return-assign
      })
    })
  }
}
...
Article.prototype.getContent = function () {
  ...
  if (this.options.selectors && this.options.selectors.content) {
    ...
    if (node && node.length > 0) {
      content = read.extract(this.$, node, sel, this.options)
    }
  } else {
    if (debug.enabled) {
