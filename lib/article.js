...
Article.prototype.contentWithTEXT = function(node, output){
  if(output.break && /^json$/i.test(output.type)){
        ...
        if(dataType.break) {
          text.split('\\n').forEach(function(txt){
            if (txt && txt.trim().length > 0) {
              retVal.push({type: 'text', value: txt});
            }
          });
        }else {
 ...
 * @return {*}
 */
Article.prototype.contentWithCHEERIO = function(node, output){
  return node;
}
